 ...
 */
class Rz(private val point: Point, private val angle: Int) : Command {
    override fun generateCommand(): String {
        return "POINT ${point.name} = ${point.name} + RZ($angle)"
    }
}

fun Program.RZ(point: Point, angle: Int) = this.add(Rz(point, angle))